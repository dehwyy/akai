...
// codebase.
// - Call and read ImGui::ShowDemoWindow() in imgui_demo.cpp. All applications in examples/ are doing
// that. Read imgui.cpp for details, links and comments.

// Resources:
// - FAQ                   http://dearimgui.com/faq
// - Homepage              https://github.com/ocornut/imgui
// - Releases & changelog  https://github.com/ocornut/imgui/releases
// - Gallery               https://github.com/ocornut/imgui/issues/6478 (please post your
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 26495)  // [Static Analyzer] Variable 'XXX' is uninitialized. Always initialize
...
#pragma GCC diagnostic ignored \
    "-Wclass-memaccess"  // [__GNUC__ >= 8] warning: 'memset/memcpy' clearing/writing an object of type
                         // 'xxxx' with no trivial copy-assignment; use assignment or
...
struct ImDrawData;  // All draw command lists required to render the frame + pos/size coordinates to use
...
struct ImDrawListSharedData;  // Data shared among multiple draw lists (typically owned by parent ImGui
                              // context, but you may create one yourself)
struct ImDrawListSplitter;  // Helper to split a draw list into different layers which can be drawn into
                            // out of order, then flattened back.
...
struct ImGuiSizeCallbackData;  // Callback data when using SetNextWindowSizeConstraints() (rare/advanced
                               // use)
...
// Enumerations
// - We don't use strongly typed enums much because they add constraints (can't extend in private code,
// can't store typed in bit fields, extra casting on iteration)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below to
// find the actual flags/enum lists!
...
enum ImGuiKey : int;          // -> enum ImGuiKey              // Enum: A key identifier (ImGuiKey_XXX or
                              // ImGuiMod_XXX value)
enum ImGuiMouseSource : int;  // -> enum ImGuiMouseSource      // Enum; A mouse input source identifier
...
typedef int ImGuiSortDirection;  // -> enum ImGuiSortDirection_   // Enum: A sorting direction (ascending
                                 // or descending)
typedef int ImGuiStyleVar;  // -> enum ImGuiStyleVar_        // Enum: A variable identifier for styling
typedef int
    ImGuiTableBgTarget;  // -> enum ImGuiTableBgTarget_   // Enum: A color target for TableSetBgColor()

// Flags (declared as int for compatibility with old C++, to allow using as flags without overhead, and
// to not pollute the top of this file)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below to
// find the actual flags/enum lists!
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas CTRL+F12
//   ("Edit.GoToImplementation") cannot. With Visual Assist installed: ALT+G
...
#ifndef ImTextureID
typedef void* ImTextureID;  // Default: store a pointer or an integer fitting in a pointer (most renderer
...
typedef unsigned short ImWchar16;  // A single decoded U16 character/code point. We encode them as multi
                                   // bytes UTF-8 when used in strings.
typedef unsigned int ImWchar32;    // A single decoded U32 character/code point. We encode them as multi
                                   // bytes UTF-8 when used in strings.
...
// This is a frequently used type in the API. Consider using IM_VEC2_CLASS_EXTRA to create implicit cast
// from/to our preferred type.
...
//-----------------------------------------------------------------------------
// [SECTION] Dear ImGui end-user API functions
// (Note that ImGui:: being a namespace, you can add extra ImGui:: functions in your own separate file.
// Please don't modify imgui source files!)
//-----------------------------------------------------------------------------
...
namespace ImGui {
    ...
    // host window. Child windows can embed their own child.
    ...
    IMGUI_API ImVec2
    GetWindowPos();  // get current window position in screen space (note: it is unlikely you need to use
    ...
    GetWindowSize();  // get current window size (note: it is unlikely you need to use this. Consider
                      // using GetScreenCursorPos() and e.g. GetContentRegionAvail() instead)
    ...
    // - You can call SameLine() between widgets to undo the last carriage return and output at the right
    // of the preceding widget.
    // - Attention! We currently have inconsistencies between window-local and absolute positions we will
    // aim to fix with future API:
    ...
    IMGUI_API void TextUnformatted(
        ...
        const char* text_end =
            NULL);  // raw text without formatting. Roughly equivalent to Text("%s", text) but: A)
                    // doesn't require null terminated string if 'text_end' is specified, B) it's faster,
                    // no memory copy is done, no buffer size limits, recommended for long chunks of
                    // text.
    ...
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to
    // query widget state.
    ...
    // display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1;
    // etc.
    // - Format string may also be set to NULL or use the default format ("%f" or "%d").
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to move by 5 pixels to increase
    // value by 1). For gamepad/keyboard navigation, minimum speed is Max(v_speed,
    // minimum_step_at_given_precision).
    // - Use v_min < v_max to clamp edits to given limits. Note that CTRL+Click manual input can override
    // those limits if ImGuiSliderFlags_AlwaysClamp is not used.
    ...
    // - CTRL+Click on any slider to turn them into an input box. Manually input values aren't clamped by
    // default and can go off-bounds. Use ImGuiSliderFlags_AlwaysClamp to always clamp.
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and
    // display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1;
    // etc.
    // - Format string may also be set to NULL or use the default format ("%f" or "%d").
    ...
    IMGUI_API bool CollapsingHeader(
        ...
        ImGuiTreeNodeFlags flags =
            0);  // when 'p_visible != NULL': if '*p_visible==true' display an additional small close
                 // button on upper right of the header which will set the bool to false when clicked, if
    ...
    // - Not that MenuItem() keyboardshortcuts are displayed as a convenience but _not processed_ by Dear
    // ImGui at the moment.
    ...
    // - BeginItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_Tooltip) &&
    // BeginTooltip())' idiom.
    // - SetItemTooltip() is a shortcut for the 'if (IsItemHovered(ImGuiHoveredFlags_Tooltip)) {
    // SetTooltip(...); }' idiom.
    // - Where 'ImGuiHoveredFlags_Tooltip' itself is a shortcut to use 'style.HoverFlagsForTooltipMouse'
    // or 'style.HoverFlagsForTooltipNav' depending on active input type. For mouse it defaults to
    ...
    //  - Their visibility state (~bool) is held internally instead of being held by the programmer as we
    ...
    //  - You can bypass the hovering restriction by using ImGuiHoveredFlags_AllowWhenBlockedByPopup when
    //  calling IsItemHovered() or IsWindowHovered().
    ...
    //  - CloseCurrentPopup() is called by default by Selectable()/MenuItem() when activated (FIXME: need
    //  some options).
    ...
    // boolean expressions. If you can avoid calling BeginDisabled(False)/EndDisabled() best to avoid it.
    ...
    // viewports.
    // - In the future we will extend this concept further to also represent Platform Monitor and support
    // a "no main platform window" operation mode.
    ...
    //  - with    IMGUI_DISABLE_OBSOLETE_KEYIO (this is the way forward): any use of ImGuiKey will assert
    //  with key < 512. GetKeyIndex() is pass-through and therefore deprecated (gone if
    ...
    IMGUI_API bool IsMouseDoubleClicked(
        ImGuiMouseButton
            button);  // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that
    ...
    IMGUI_API void SetNextFrameWantCaptureMouse(
        bool want_capture_mouse);  // Override io.WantCaptureMouse flag next frame (said flag is left for
                                   // your application to handle, typical when true it instucts your app
                                   // to ignore inputs). This is equivalent to setting
                                   // "io.WantCaptureMouse = want_capture_mouse;" after the next
                                   // NewFrame() call.

    // Clipboard Utilities
    // - Also see the LogToClipboard() function to capture GUI into clipboard, or easily output text data
    // to the clipboard.
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NoScrollWithMouse =
        1 << 4,  // Disable user vertically scrolling with mouse wheel. On child window, mouse wheel will
                 // be forwarded to the parent unless NoScrollbar is also set.
    ImGuiWindowFlags_NoCollapse =
        1 << 5,  // Disable user collapsing window by double-clicking on it. Also referred to as Window
    ...
    ImGuiWindowFlags_UnsavedDocument =
        1 << 20,  // Display a dot next to the title. When used in a tab/docking context, tab is selected
                  // when clicking the X + closure is not assumed (will wait for user to stop submitting
                  // the tab). Otherwise closure is assumed when pressing the X, so if you keep
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_NoUndoRedo =
        1 << 16,  // Disable undo/redo. Note that input text owns the text data while active, if you want
                  // to provide your own undo/redo stack you need e.g. to call ClearActiveID().
    ...
    ImGuiInputTextFlags_CallbackResize =
        1 << 18,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value),
                  // allowing the string to grow. Notify when the string wants to be resized (for string
                  // types which hold a cache of their Size). You will be provided a new BufSize in the
                  // callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using
                  // this)
    ...
    // Obsolete names
    // ImGuiInputTextFlags_AlwaysInsertMode  = ImGuiInputTextFlags_AlwaysOverwrite   // [renamed in 1.82]
    // name was not matching behavior
};
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_OpenOnArrow =
        1 << 7,  // Only open when clicking on the arrow part. If ImGuiTreeNodeFlags_OpenOnDoubleClick is
                 // also set, single-click arrow or double-click all box to open.
    ImGuiTreeNodeFlags_Leaf = 1 << 8,  // No collapsing, no arrow (use as a convenience for leaf nodes).
    ImGuiTreeNodeFlags_Bullet = 1 << 9,  // Display a bullet instead of arrow. IMPORTANT: node can still
                                         // be marked open/close if you don't set the _Leaf flag!
    ImGuiTreeNodeFlags_FramePadding =
        1 << 10,  // Use FramePadding (even for an unframed text node) to vertically align text baseline
                  // to regular widget height. Equivalent to calling AlignTextToFramePadding().
    ImGuiTreeNodeFlags_SpanAvailWidth =
        1 << 11,  // Extend hit box to the right-most edge, even if not framed. This is not the default
                  // in order to allow adding other items on the same line. In the future we may refactor
                  // the hit system to be front-to-back, allowing natural overlaps and then this can
                  // become the default.
    ImGuiTreeNodeFlags_SpanFullWidth =
        1 << 12,  // Extend hit box to the left-most and right-most edges (bypass the indented area).
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1 << 13,  // (WIP) Nav: left direction may move to this TreeNode() from any of its child (items
...
// - To be backward compatible with older API which took an 'int mouse_button = 1' argument, we need to
// treat
//   small flags values as a mouse button index, so we encode the mouse button in the first few bits of
//   the flags. It is therefore guaranteed to be legal to pass a mouse button index in ImGuiPopupFlags.
// - For the same reason, we exceptionally default the ImGuiPopupFlags argument of BeginPopupContextXXX
// functions to 1 instead of 0.
//   IMPORTANT: because the default parameter is 1 (==ImGuiPopupFlags_MouseButtonRight), if you rely on
//   the default parameter and want to use another flag, you need to pass in the
//   ImGuiPopupFlags_MouseButtonRight flag explicitly.
// - Multiple buttons currently cannot be combined/or-ed in those functions (we could allow it later).
enum ImGuiPopupFlags_ {
    ImGuiPopupFlags_None = 0,
    ImGuiPopupFlags_MouseButtonLeft = 0,  // For BeginPopupContext*(): open on Left Mouse release.
                                          // Guaranteed to always be == 0 (same as ImGuiMouseButton_Left)
    ImGuiPopupFlags_MouseButtonRight =
        1,  // For BeginPopupContext*(): open on Right Mouse release. Guaranteed to always be == 1 (same
            // as ImGuiMouseButton_Right)
    ImGuiPopupFlags_MouseButtonMiddle =
        2,  // For BeginPopupContext*(): open on Middle Mouse release. Guaranteed to always be == 2 (same
            // as ImGuiMouseButton_Middle)
    ImGuiPopupFlags_MouseButtonMask_ = 0x1F,
    ImGuiPopupFlags_MouseButtonDefault_ = 1,
    ImGuiPopupFlags_NoOpenOverExistingPopup =
        1 << 5,  // For OpenPopup*(), BeginPopupContext*(): don't open if there's already a popup at the
                 // same level of the popup stack
    ...
    ImGuiPopupFlags_AnyPopupLevel = 1 << 8,  // For IsPopupOpen(): search/test at any level of the popup
...
enum ImGuiComboFlags_ {
    ...
    ImGuiComboFlags_HeightSmall =
        1 << 1,  // Max ~4 items visible. Tip: If you want your combo popup to be a specific size you can
                 // use SetNextWindowSizeConstraints() prior to calling BeginCombo()
...
enum ImGuiTabBarFlags_ {
    ...
    ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
        1 << 3,  // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle
                 // mouse button. You can still repro this behavior on user's side with if
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument =
        1
        << 0,  // Display a dot next to the title + tab is selected when clicking the X + closure is not
               // assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed
    ...
    ImGuiTabItemFlags_NoCloseWithMiddleMouseButton =
        1 << 2,  // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle
                 // mouse button. You can still repro this behavior on user's side with if
                 // (IsItemHovered() && IsMouseClicked(2)) *p_open = false.
...
//    - Fixed Columns (if any) will generally obtain their requested width (unless the table cannot fit
//    them all).
//    - Stretch Columns will share the remaining width according to their respective weight.
//    - Mixed Fixed/Stretch columns is possible but has various side-effects on resizing behaviors.
//      The typical use of mixing sizing policies is: any number of LEADING Fixed columns, followed by
//      one or two TRAILING Stretch columns. (this is because the visible order of columns have subtle
//      but necessary effects on how they react to manual resizing).
// - When ScrollX is on:
...
//    - Using Stretch columns OFTEN DOES NOT MAKE SENSE if ScrollX is on, UNLESS you have specified a
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_ContextMenuInBody =
        1 << 5,  // Right-click on columns body/contents will display table context menu. By default it
                 // is available in TableHeadersRow(). Decorations
    ...
    ImGuiTableFlags_NoBordersInBodyUntilResize =
        1 << 12,  // [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders
                  // will always appear in Headers). -> May move to style Sizing Policy (read above for
                  // defaults)
    ...
    ImGuiTableFlags_SizingStretchSame =
        4 << 13,  // Columns default to _WidthStretch with default weights all equal, unless overridden
                  // by TableSetupColumn(). Sizing Extra Options
    ImGuiTableFlags_NoHostExtendX =
        1 << 16,  // Make outer width auto-fit to columns, overriding outer_size.x value. Only available
    ...
    ImGuiTableFlags_NoHostExtendY =
        1 << 17,  // Make outer height stop exactly at outer_size.y (prevent auto-extending table past
                  // the limit). Only available when ScrollX/ScrollY are disabled. Data below the limit
                  // will be clipped and not visible.
    ImGuiTableFlags_NoKeepColumnsVisible =
        1 << 18,  // Disable keeping column always minimally visible when ScrollX is off and table gets
                  // too small. Not recommended if columns are resizable.
    ImGuiTableFlags_PreciseWidths =
        1
        << 19,  // Disable distributing remainder width to stretched columns (width allocation on a
                // 100-wide table with 3 columns: Without this flag: 33,33,34. With this flag: 33,33,33).
                // With larger number of columns, resizing will appear to be less smooth. Clipping
    ImGuiTableFlags_NoClip =
        1 << 20,  // Disable clipping rectangle for every individual columns (reduce draw command count,
    ...
    ImGuiTableFlags_NoPadInnerX =
        1 << 23,  // Disable inner padding between columns (double inner padding if BordersOuterV is on,
                  // single inner padding if BordersOuterV is off). Scrolling
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_WidthStretch =
        1 << 3,  // Column will stretch. Preferable with horizontal scrolling disabled (default if table
    ...
    ImGuiTableColumnFlags_NoClip = 1 << 8,  // Disable clipping for this column (all NoClip columns will
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12,  // TableHeadersRow() will not submit label for this column. Convenient for some small
    ...
    ImGuiTableColumnFlags_IsEnabled = 1 << 24,  // Status: is enabled == not hidden by user/api (referred
...
enum ImGuiTableRowFlags_ {
    ...
    ImGuiTableRowFlags_Headers = 1 << 0,  // Identify header row (set default background color + width of
...
// The purpose of the two row/columns layers is to let you decide if a background color change should
// override or blend with the existing color. When using ImGuiTableFlags_RowBg on the table, each row has
// the RowBg0 color automatically set for odd/even rows. If you set the color of RowBg0 target, your
// color will override the existing RowBg0 color. If you set the color of RowBg1 or ColumnBg1 target,
...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_AnyWindow =
        1 << 2,  // Return true if any window is focused. Important: If you are trying to tell how to
                 // dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse' instead!
                 // Please read the FAQ!
...
// you should use 'io.WantCaptureMouse' instead! Please read the FAQ! Note: windows with the
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_NoPopupHierarchy =
        1 << 3,  // IsWindowHovered() only: Do not consider popup hierarchy (do not treat popup emitter
                 // as parent of popup) (when used with _ChildWindows or _RootWindow)
    // ImGuiHoveredFlags_DockHierarchy               = 1 << 4,   // IsWindowHovered() only: Consider
    // docking hierarchy (treat dockspace host as parent of docked window) (when used with _ChildWindows
    // or _RootWindow)
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByActiveItem =
        1 << 7,  // Return true even if an active item is blocking access to this item/window. Useful for
    ...
    ImGuiHoveredFlags_AllowWhenOverlappedByWindow =
        1 << 9,  // IsItemHovered() only: Return true even if the position is obstructed or overlapped by
                 // another window.
    ...
    ImGuiHoveredFlags_Stationary =
        1 << 13,  // Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least
                  // one time_. After this, can move on same item/window. Using the stationary test tends
    ...
    ImGuiHoveredFlags_DelayNormal =
        1 << 16,  // IsItemHovered() only: Return true after style.HoverDelayNormal elapsed (~0.40 sec)
                  // (shared between items) + requires mouse to be stationary for
                  // style.HoverStationaryDelay (once per item).
    ImGuiHoveredFlags_NoSharedDelay =
        1 << 17,  // IsItemHovered() only: Disable shared delay system where moving from one item to the
                  // next keeps the previous timer for a short time (standard for tooltips with long
                  // delays)
};
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceNoPreviewTooltip =
        1 << 0,  // Disable preview tooltip. By default, a successful call to BeginDragDropSource opens a
                 // tooltip so you can display a preview or description of the source contents. This flag
                 // disables this behavior.
    ImGuiDragDropFlags_SourceNoDisableHover =
        1 << 1,  // By default, when dragging we clear data so that IsItemHovered() will return false, to
                 // avoid subsequent user code submitting tooltips. This flag disables this behavior so
                 // you can still call IsItemHovered() on the source item.
    ImGuiDragDropFlags_SourceNoHoldToOpenOthers =
        1 << 2,  // Disable the behavior that allows to open tree nodes and collapsing header by holding
                 // over them while dragging a source item.
    ImGuiDragDropFlags_SourceAllowNullID =
        1 << 3,  // Allow items such as Text(), Image() that have no unique identifier to be used as drag
                 // source, by manufacturing a temporary identifier based on their window-relative
                 // position. This is extremely unusual within the dear imgui ecosystem and so we made it
                 // explicit.
    ImGuiDragDropFlags_SourceExtern =
        1
        << 4,  // External source (from outside of dear imgui), won't attempt to read current item/window
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery =
        1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released.
...
// Standard Drag and Drop payload types. You can define you own payload types using short strings. Types
// starting with '_' are defined by Dear ImGui.
...
// values. All our named keys are >= 512. Keys value 0 to 511 are left unused as legacy native/opaque key
// values (< 1.87). Since >= 1.89 we increased typing (went from int to enum), some legacy code may need
// a cast to ImGuiKey. Read details about the 1.87 and 1.89 transition :
// https://github.com/ocornut/imgui/issues/4921 Note that "Keys" related to physical keys and are not the
// same concept as input "Characters", the later are submitted via io.AddInputCharacter().
enum ImGuiKey : int {
    // Keyboard
    ImGuiKey_None = 0,
    ...
    ImGuiKey_GamepadFaceLeft,   // X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu.
    ...
    ImGuiKey_GamepadDpadRight,  // D-pad Right                                      // Move / Tweak /
                                // Resize Window (in Windowing mode)
    ImGuiKey_GamepadDpadUp,  // D-pad Up                                         // Move / Tweak / Resize
    ...
    ImGuiKey_GamepadR1,  // R Bumper (Xbox)  R (Switch)   R1 (PS)            // Tweak Faster / Focus Next
                         // (in Windowing mode)
    ...
    // - This is mirroring the data also written to io.MouseDown[], io.MouseWheel, in a format allowing
    // them to be accessed via standard key API.
    ImGuiKey_MouseLeft,
    ImGuiKey_MouseRight,
    ...
    // - This is mirroring the data also written to io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper, in a
    // format allowing
    //   them to be accessed via standard key API, allowing calls such as IsKeyPressed(),
    //   IsKeyReleased(), querying duration etc.
    // - Code polling every key (e.g. an interface to detect a key press for input mapping) might want to
    // ignore those
    ...
    //   In practice: it's complicated; mods are often provided from different sources. Keyboard layout,
    //   IME, sticky keys and backends tend to interfere and break that equivalence. The safer decision
    //   is to relay that ambiguity down to the end-user...
    ImGuiMod_None = 0,
    ...
    // the io.KeyMap[] array. We are ditching this method but keeping a legacy path for user code doing
    // e.g. IsKeyPressed(MY_NATIVE_KEY_CODE) If you need to iterate all keys (for e.g. an input mapper)
    // you may use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableSetMousePos =
        1
        << 2,  // Instruct navigation to move the mouse cursor. May be useful on TV/console systems where
               // moving a virtual mouse is awkward. Will update io.MousePos and set
               // io.WantSetMousePos=true. If enabled you MUST honor io.WantSetMousePos requests in your
               // backend, otherwise ImGui will react as if the mouse is jumping around back and forth.
    ImGuiConfigFlags_NavNoCaptureKeyboard =
        1 << 3,  // Instruct navigation to not set the io.WantCaptureKeyboard flag when io.NavActive is
                 // set.
    ImGuiConfigFlags_NoMouse = 1 << 4,  // Instruct imgui to clear mouse position/buttons in NewFrame().
                                        // This allows ignoring the mouse information set by the backend.
    ImGuiConfigFlags_NoMouseCursorChange =
        1 << 5,  // Instruct backend to not alter mouse cursor shape and visibility. Use if the backend
                 // cursor changes are interfering with yours and you don't want to use SetMouseCursor()
                 // to change mouse cursor. You may want to honor requests from imgui by reading
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_RendererHasVtxOffset =
        1 << 3,  // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes
...
// - Tip: Use your programming IDE navigation facilities on the names in the _second column_ below to
// find the actual members and their description.
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas CTRL+F12
//   ("Edit.GoToImplementation") cannot. With Visual Assist installed: ALT+G
//   ("VAssistX.GoToImplementation") can also follow symbols in comments.
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoOptions = 1 << 3,  //              // ColorEdit: disable toggling options menu
    ...
    ImGuiColorEditFlags_NoInputs =
        1 << 5,  //              // ColorEdit, ColorPicker: disable inputs sliders/text widgets (e.g. to
    ...
    ImGuiColorEditFlags_HDR =
        1 << 19,  //              // (WIP) ColorEdit: Currently only disable 0.0f..1.0f limits in RGBA
                  //              edition (note: you probably want to use ImGuiColorEditFlags_Float flag
                  //              as well).
    ImGuiColorEditFlags_DisplayRGB =
        1 << 20,  // [Display]    // ColorEdit: override _display_ type among RGB/HSV/Hex. ColorPicker:
    ...
    // Defaults Options. You can set application defaults using SetColorEditOptions(). The intent is that
    // you probably don't want to override them in most of your calls. Let the user choose via the option
...
// We use the same sets of flags for DragXXX() and SliderXXX() functions as the features are the same and
// it makes it easier to swap them. (Those are per-item flags. There are shared flags in ImGuiIO:
...
// User code may request backend to display given cursor by calling SetMouseCursor(), which is why we
// have some cursors that are marked unused here
...
// IsMousePressed(), io.AddMousePosEvent() But that "Mouse" data can come from different source which
// occasionally may be useful for application to know about. You can submit a change of pointer type
// using io.AddMouseSourceEvent().
enum ImGuiMouseSource : int {
    ImGuiMouseSource_Mouse = 0,    // Input is coming from an actual mouse.
    ImGuiMouseSource_TouchScreen,  // Input is coming from a touch screen (no hovering prior to initial
...
// Important: Treat as a regular enum! Do NOT combine multiple values using binary operators! All the
...
enum ImGuiCond_ {
    ...
    ImGuiCond_FirstUseEver = 1 << 2,  // Set the variable if the object/window has no persistently saved
...
// Defining a custom placement new() with a custom parameter allows us to bypass including <new> which on
// some platforms complains when user has disabled exceptions.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// ImVector<>
// Lightweight std::vector<>-like class to avoid dragging dependencies (also, some implementations of STL
// with debug enabled are absurdly slow, we bypass it so our code runs fast in debug).
//-----------------------------------------------------------------------------
...
// - Important: our implementation does NOT call C++ constructors/destructors, we treat everything as raw
// data! This is intentional but be extra mindful of that,
//   Do NOT use this class as a std::vector replacement in your own code! Many of the structures used by
//   dear imgui can be safely initialized by a zero-memset.
//-----------------------------------------------------------------------------
...
// During the frame, use ImGui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style
...
struct ImGuiStyle {
        ...
        ImVec2 CellPadding;        // Padding within a table cell. CellPadding.y may be altered between
                                   // different rows.
...
struct ImGuiIO {
        ...
        // regardless of return value of BeginXXX() This is inconsistent with other BeginXXX functions
        // and create confusion for many users. We expect to update the API eventually. In the meanwhile
        // we provide tools to facilitate checking user-code behavior.
        ...
        // debugger when focus loss leads to clearing inputs data. Backends may have other side-effects
        // on focus loss, so this will reduce side-effects but not necessary remove all of them. Consider
        // using e.g. Win32's IsDebuggerPresent() as an additional filter (or see ImOsIsDebuggerPresent()
        // in imgui_test_engine/imgui_te_utils.cpp for a Unix compatible version).
        ...
        ImWchar
            PlatformLocaleDecimalPoint;  // '.'              // [Experimental] Configure decimal point
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
        IMGUI_API void ClearInputCharacters();  // [Obsolete] Clear the current frame text input buffer.
                                                // Now included within ClearInputKeys().
#endif
...
// returns true on edit, the callback is useful mainly to manipulate the underlying buffer while focus is
// active)
...
// Callback is called during the next Begin(). NB: For basic min/max size constraint on each axis you
// don't need to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
...
// When 'SpecsDirty == true' you can sort your data. It will be true with sorting specs have changed
...
// Helper: Execute a block of code at maximum once a frame. Convenient if you want to quickly create a UI
// within deep-nested code that runs multiple times every frame. Usage: static ImGuiOnceUponAFrame oaf;
// if (oaf) ImGui::Text("This will be called only once per frame");
...
// (typically tied to user interactions aka max once a frame) You can use it as custom user storage for
// temporary values. Declare your own storage if, for example:
// - You want to manipulate the open/close state of a particular sub-tree in your interface (tree node
// uses Int 0/1 to store their state).
// - You want to store custom debug data easily without adding or editing structures in your code
// (probably not efficient, but convenient) Types are NOT stored, so it is up to you to make sure your
// Key don't collide with different types.
struct ImGuiStorage {
        // [Internal]
...
// non-visible items we have skipped. (Dear ImGui already clip items based on their bounds but: it needs
// to first layout the item to do so, and generally
...
// - Clipper lets you process the first element (DisplayStart = 0, DisplayEnd = 1) regardless of it being
// visible or not.
...
struct ImGuiListClipper {
        ...
        // of certain items it is possible that an extra item may be included on either end of the
        // display range).
...
// - This is in order to allow user enabling implicit cast operators between ImVec2/ImVec4 and their own
// types (using IM_VEC2_CLASS_EXTRA in imconfig.h)
...
// Prefer using IM_COL32() macros if you want a guaranteed compile-time ImU32 for usage with ImDrawList
// API.
// **Avoid storing ImColor! Store either u32 of ImVec4. This is not a full-featured color class. MAY
// OBSOLETE.
// **None of the ImGui API are using ImColor directly but you can use it as a convenience to pass colors
// in either ImU32 or ImVec4 formats. Explicitly cast to ImU32 or ImVec4 if needed.
struct ImColor {
        ImVec4 Value;
...
// NB: You most likely do NOT need to use draw callbacks just to create your own widget or customized UI
// rendering, you can poke into the draw list for that! Draw callback may be useful for example to:
...
// function at this address. This is useful for example if you submitted callbacks which you know have
// altered the render state and you want it to be restored. It is not done by default because they are
// many perfectly useful way of altering render state for imgui contents (e.g. changing shader/blending
...
// - The ClipRect/TextureId/VtxOffset fields must be contiguous as we memcmp() them together (this is
// asserted for).
struct ImDrawCmd {
...
// imconfig.h The code expect ImVec2 pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can
// re-order them or add other fields as needed to simplify integration in your engine. The type has to be
// described within the macro (you can either declare the struct or use a typedef). This is because
// ImVec2/ImU32 are likely not declared at the time you'd want to set your type up. NOTE: IMGUI DOESN'T
// CLEAR THE STRUCTURE AND DOESN'T CALL A CONSTRUCTOR SO ANY CUSTOM FIELD WILL BE UNINITIALIZED. IF YOU
// ADD EXTRA FIELDS (SUCH AS A 'Z' COORDINATES) YOU WILL NEED TO CLEAR THEM DURING RENDER OR TO IGNORE
// THEM.
IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT;
#endif
...
// Split/Merge functions are used to split the draw list into different layers which can be drawn into
// out of order. This is used by the Columns/Tables API, so items of each column can be batched together
// in a same draw call.
...
// (Legacy: bit 0 must always correspond to ImDrawFlags_Closed to be backward compatible with old API
// using a bool. Bits 1..3 must be unused)
enum ImDrawFlags_ {
    ImDrawFlags_None = 0,
    ...
    ImDrawFlags_RoundCornersTopRight =
        1 << 5,  // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right corner only (when
                 // rounding > 0.0f, we default to all corners). Was 0x02.
    ImDrawFlags_RoundCornersBottomLeft =
        1 << 6,  // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-left corner only (when
    ...
    ImDrawFlags_RoundCornersDefault_ =
        ImDrawFlags_RoundCornersAll,  // Default to ALL corners if none of the _RoundCornersXX flags are
                                      // specified.
    ImDrawFlags_RoundCornersMask_ = ImDrawFlags_RoundCornersAll | ImDrawFlags_RoundCornersNone,
};

// Flags for ImDrawList instance. Those are set automatically by ImGui:: functions from ImGuiIO settings,
// and generally not manipulated directly. It is however possible to temporarily alter flags between
...
// transformation matrix to want to the data (depending on the use of the transformation you may want to
// apply it to ClipRect as well!) Important: Primitives are always added to the list and not culled
...
struct ImDrawList {
        ...
        // ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3, p4,
        // col, thickness, num_segments); } // OBSOLETED in 1.80 (Jan 2021) inline  void
...
// (NB: the style and the naming convention here is a little inconsistent, we currently preserve them for
// backward compatibility purpose, as this is one of the oldest structure exposed by the library!
// Basically, ImDrawList == CmdList)
struct ImDrawData {
...
// (Note: some language parsers may fail to convert the 31+1 bitfield members, in this case maybe drop
...
// Helper to build glyph ranges from text/string data. Feed your application strings/characters to it
// then call BuildRanges(). This is essentially a tightly packed of vector of 64k booleans = 8KB storage.
...
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded
//  in the code will be loaded for you.
//  - Call GetTexDataAsAlpha8() or GetTexDataAsRGBA32() to build and retrieve pixels data.
//  - Upload the pixels data into a texture within your graphics system (see imgui_impl_xxxx.cpp
//  examples)
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture in a format natural to
//  your graphics API.
...
// Common pitfalls:
// - If you pass a 'glyph_ranges' array to AddFont*** functions, you need to make sure that your array
// persist up until the
//   atlas is build (when calling GetTexData*** or Build()). We only copy the pointer, not the data.
// - Important: By default, AddFontFromMemoryTTF() takes ownership of the data. Even though we are not
// writing to it, we will free the pointer on destruction.
//   You can set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be
//   freed,
// - Even though many functions are suffixed with "TTF", OTF data is supported just as well.
// - This is an old API and it is currently awkward for those and various other reasons! We will address
// them in the future!
struct ImFontAtlas {
...
//   - Work Area = entire viewport minus sections used by main menu bars (for platform windows), or by
//   task bar (for platform monitor).
...
namespace ImGui {
    ...
    // GetStyle().FramePadding.y * 2.0f; return BeginListBox(label, ImVec2(0.0f, height)); } // Helper to
    // calculate size from items_count and height_in_items static inline void  ListBoxFooter() {
    // EndListBox(); }
    ...
    // ImGuiDataType data_type, void* p_data, int components, const void* p_min, const void* p_max, const
    ...
    // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat2(const char* label, float v[2],
    // float v_min, float v_max, const char* format, float power = 1.0f)              { return
    // SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, power); }        //
    // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat3(const char* label, float v[3],
    // float v_min, float v_max, const char* format, float power = 1.0f)              { return
    // SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, power); }        //
    // OBSOLETED in 1.78 (from June 2020) static inline bool  SliderFloat4(const char* label, float v[4],
    // float v_min, float v_max, const char* format, float power = 1.0f)              { return
    ...
    // and Apr 2018) static inline void  ShowTestWindow()                      { return ShowDemoWindow();
    // }                                        // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017)
    // static inline bool  IsRootWindowFocused()                 { return
    ...
    // bg_alpha = -1.0f, ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017):
    // Equivalent of using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha().
    // static inline bool  IsRootWindowOrAnyChildHovered()       { return
    ...
    // (between Aug 2017 and Oct 2017) static inline bool  IsItemHoveredRect()                   { return
    // IsItemHovered(ImGuiHoveredFlags_RectOnly); }               // OBSOLETED in 1.51 (between Jun 2017
    // and Aug 2017) static inline bool  IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0); return
    // false; }                                     // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017):
    // This was misleading and partly broken. You probably want to use the io.WantCaptureMouse flag
    // instead. static inline bool  IsMouseHoveringAnyWindow()            { return
    // IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED in 1.51 (between Jun 2017
    // and Aug 2017) static inline bool  IsMouseHoveringWindow()               { return
    ...
    // GetFontSize(); }                                           // OBSOLETED in 1.48 static inline void
    // SetScrollPosHere()                    { SetScrollHere(); } // OBSOLETED in 1.42
}  // namespace ImGui
...
// RENAMED ImGuiKeyModFlags -> ImGuiModFlags in 1.88 (from April 2022). Exceptionally commented out ahead
// of obscolescence schedule to reduce confusion and because they were not meant to be used in the first
// place.
typedef ImGuiKeyChord
    ImGuiModFlags;  // == int. We generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any number
...
// ImGuiKeyModFlags_Shift = ImGuiMod_Shift, ImGuiKeyModFlags_Alt = ImGuiMod_Alt, ImGuiKeyModFlags_Super =
// ImGuiMod_Super };
...
// Include imgui_user.h at the end of imgui.h (convenient for user to only explicitly include vanilla
// imgui.h)
