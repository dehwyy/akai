...
namespace std {
    ...
    struct char_traits<unsigned int> {
            ...
            static int_type not_eof(int_type c) noexcept { return c != EOF ? c : ~EOF; }
...
namespace ImGui {
    namespace SFML {
        ...
        void SetCurrentWindow(const sf::Window& window) {
            auto found =
                std::find_if(s_windowContexts.begin(),
    ...
    void Image(const sf::Sprite& sprite,
        ...
        ImGui::Image(
            textureID, ImVec2(size.x, size.y), uv0, uv1, toImColor(tintColor), toImColor(borderColor));
    }
    ...
    bool ImageButton(const sf::Texture& texture,
                     ...
                     const sf::Color& tintColor) {
        return ImageButton(
            texture, static_cast<sf::Vector2f>(texture.getSize()), framePadding, bgColor, tintColor);
    }
    ...
    bool ImageButton(const sf::RenderTexture& texture,
                     const int framePadding,
                     const sf::Color& bgColor,
                     const sf::Color& tintColor) {
        return ImageButton(
    ...
    bool ImageButton(const sf::Sprite& sprite,
        ...
        return ImageButton(
            sprite, sf::Vector2f(spriteSize.width, spriteSize.height), framePadding, bgColor, tintColor);
    }
    ...
    void DrawLine(const sf::Vector2f& a,
                  const sf::Vector2f& b,
    ...
    void DrawRect(const sf::FloatRect& rect,
                  const sf::Color& color,
                  float rounding,
    ...
    void DrawRectFilled(const sf::FloatRect& rect,
                        const sf::Color& color,
                        float rounding,
                        int rounding_corners) {
        ImDrawList* draw_list = ImGui::GetWindowDrawList();
...
namespace {
    ImColor toImColor(sf::Color c) {
        return ImColor(
            static_cast<int>(c.r), static_cast<int>(c.g), static_cast<int>(c.b), static_cast<int>(c.a));
    }
    ImVec2 getTopLeftAbsolute(const sf::FloatRect& rect) {
        ImVec2 pos = ImGui::GetCursorScreenPos();
    ...
    void updateJoystickButtonState(ImGuiIO& io) {
        for (int i = 0; i < static_cast<int>(sf::Joystick::ButtonCount); ++i) {
            ...
            if (key != ImGuiKey_None) {
                bool isPressed =
                    sf::Joystick::isButtonPressed(s_currWindowCtx->joystickId, static_cast<unsigned>(i));
    ...
    const char* getClipboardText(void* /*userData*/) {
        return "null";
        // auto temp = sf::Clipboard::getString().toUtf8();
        // std::string utf8(reinterpret_cast<const char*>(temp.data()), temp.size());
        // std::string tmp(reinterpret_cast<const char*>(utf8.data()), utf8.size());
